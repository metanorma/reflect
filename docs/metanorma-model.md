...
`isodoc-presentation.rnc` opens with `include "isodoc.rnc" { … }` (overrides
inside the braces), then extends after the closing brace with RelaxNG combines:
`&=` (interleave — adds attributes/children) and `|=` (choice — adds
alternatives).
...
authored by the presentation transform and must never be emitted by the editor.
...
One value-transforming override matters: `IdRefType` is plain `text` in
Semantic (cross-references point at `@anchor`) but `xsd:IDREF` in Presentation
(pointing at `@id`). Content-dropping reductions like those above need no
editor attention — the transform performs the removal; value-transforming
overrides like this one do — the editor must author the Semantic form (§6).
